...
public class DefCallResult implements TResult {
  ...
  public Expression getType(@Nullable Equations equations) {
    if (myResultType instanceof UniverseExpression universe && !(universe.getSortExpression() instanceof SortExpression.Const) && !myArguments.isEmpty()) {
      ...
      return new UniverseExpression(universe.getSortExpression().subst(false, myDefinition.getTopLevelDefinition().getSortLevelArguments(myArguments), LevelSubstitution.EMPTY));
    }
    return myResultType;
  }
