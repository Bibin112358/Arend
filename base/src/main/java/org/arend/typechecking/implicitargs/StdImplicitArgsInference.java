    ...
    if (definition == Prelude.ARRAY_CONS && defCallResult != null && defCallResult.getArguments().isEmpty() && index2 < arguments.size()) {
      ...
      if (elementsType != null) {
        ...
        if (var == null) {
          ...
          if (constType != null) {
            ...
            argClassCall = new ClassCallExpression(Prelude.DEP_ARRAY, defCallResult.getLevels(), impls, defCallResult.getLevels().toLevelPair().toSort().max(Sort.SET0), UniverseKind.NO_UNIVERSES);
      ...
      if (!checked) {
        ...
        if (!checked) {
          ...
          Expression expected2 = new ClassCallExpression(Prelude.DEP_ARRAY, defCallResult.getLevels(), impls, defCallResult.getLevels().toLevelPair().toSort(), UniverseKind.NO_UNIVERSES);
      ...
      if (length == null) {
        ...
        type = new ClassCallExpression(Prelude.DEP_ARRAY, defCallResult.getLevels(), impls, defCallResult.getLevels().toLevelPair().toSort(), UniverseKind.NO_UNIVERSES);
      }
      ...
      Expression resultExpr = new LamExpression(lamParam, ArrayExpression.make(defCallResult.getLevels().toLevelPair(), elementsType, new SingletonList<>(result1.expression), new ReferenceExpression(lamParam)));
