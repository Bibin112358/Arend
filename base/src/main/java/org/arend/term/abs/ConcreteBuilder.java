...
public class ConcreteBuilder implements AbstractDefinitionVisitor<Concrete.ResolvableDefinition>, AbstractExpressionVisitor<Void, Concrete.Expression>, AbstractLevelExpressionVisitor<Void, Concrete.LevelExpression> {
  ...
  private InternalReferableImpl convertInternalReferable(Abstract.AbstractLocatedReferable referable, TCDefReferable parent, boolean isVisible) {
  ...
  private LocatedReferableImpl convertField(Abstract.ClassField classField, TCDefReferable parent) {
    ...
    for (Concrete.Parameter parameter : parameters) {
      for (Referable referable : parameter.getReferableList()) {
        if (referable != null && !eliminated.contains(referable.getRefName())) {
          result.add(new ParameterReferable(definition.getData(), i, referable, null));
        }
        i++;
      }
    }
  ...
  public Concrete.MetaDefinition visitMeta(Abstract.MetaDefinition def) {
    ...
    return new Concrete.MetaDefinition((MetaReferable) myDefinition, visitLevelParameters(def.getLevelParameters()), buildParameters(def.getParameters(), false, true), term != null ? term.accept(this, null) : null);
  }
  ...
  public Concrete.BaseFunctionDefinition visitFunction(Abstract.FunctionDefinition def) {
    ...
    List<Concrete.Parameter> parameters = buildParameters(def.getParameters(), true, false);
  ...
  public Concrete.DataDefinition visitData(Abstract.DataDefinition def) {
    ...
    return new Concrete.DataDefinition(myDefinition, visitLevelParameters(def.getLevelParameters()), typeParameters, elimExpressions == null ? null : buildReferences(elimExpressions), def.isTruncated(), universe instanceof Concrete.UniverseExpression ? (Concrete.UniverseExpression) universe : null, clauses);
  }
  ...
  public void buildClassParameters(Collection<? extends Abstract.FieldParameter> absParameters, Concrete.ClassDefinition classDef, List<Concrete.ClassElement> elements) {
    for (Abstract.FieldParameter absParameter : absParameters) {
      Concrete.Parameter parameter = buildParameter(absParameter, false, false, true);
  ...
  public Concrete.Definition visitClass(Abstract.ClassDefinition def) {
    ...
    Concrete.ClassDefinition classDef = new Concrete.ClassDefinition(myDefinition, visitLevelParameters(def.getLevelParameters()), def.isRecord(), def.withoutClassifying(), buildReferenceExpressions(def.getSuperClasses()), elements);
  ...
  public List<Concrete.ReferenceExpression> buildReferenceExpressions(Collection<? extends Abstract.ReferenceExpression> absElimExpressions) {
    ...
    for (Abstract.ReferenceExpression expr : absElimExpressions) {
      elimExpressions.add(new Concrete.ReferenceExpression(expr.getData(), expr.getReferent(), visitLevels(expr.getLevels())));
    }
    return elimExpressions;
  }
  ...
  public Concrete.Parameter buildParameter(Abstract.Parameter parameter, boolean isNamed, boolean isDefinition, boolean allowNullRefs) {
    ...
    if (!isNamed && (referableList.isEmpty() || referableList.size() == 1 && referableList.getFirst() == null)) {
      if (isDefinition && isStrict || !allowNullRefs) {
        return new Concrete.DefinitionTelescopeParameter(parameter.getData(), parameter.isExplicit(), true, Collections.singletonList(allowNullRefs ? null : new DataLocalReferable(parameter, null)), cType, isProperty);
      } else {
        if (isStrict) {
  ...
  public List<Concrete.Parameter> buildParameters(Collection<? extends Abstract.Parameter> absParameters, boolean isDefinition, boolean allowNullRefs) {
    List<Concrete.Parameter> parameters = new ArrayList<>(absParameters.size());
    for (Abstract.Parameter absParameter : absParameters) {
      parameters.add(buildParameter(absParameter, true, isDefinition, allowNullRefs));
    }
    return parameters;
  }
  ...
  public List<Concrete.TypeParameter> buildTypeParameters(Collection<? extends Abstract.Parameter> absParameters, boolean isDefinition) {
    ...
    for (Abstract.Parameter absParameter : absParameters) {
      Concrete.Parameter parameter = buildParameter(absParameter, false, isDefinition, true);
  ...
  private List<Concrete.Pattern> buildLamParameters(Collection<? extends Abstract.LamParameter> parameters, List<Concrete.Parameter> cParams) {
    ...
    for (Abstract.LamParameter parameter : parameters) {
      if (parameter instanceof Abstract.Parameter) {
        cParams.add(buildParameter((Abstract.Parameter) parameter, true, false, true));
    ...
    return left == null && right == null
      ...
      : left == null
        ...
        : right == null
          ...
          : new Concrete.MaxLevelExpression(data, left.accept(this, param), right.accept(this, param));
  }
