...
public class RecreateExpressionVisitor extends SubstVisitor {
  ...
  public Expression visitClassCall(ClassCallExpression expr, Void params) {
    ...
    ClassCallExpression classCall = new ClassCallExpression(expr.getDefinition(), expr.getLevels().subst(getLevelSubstitution()), fieldSet, expr.getSortExpression().subst(getLevelSubstitution()), expr.getUniverseKind());
