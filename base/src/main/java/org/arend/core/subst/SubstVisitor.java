...
public class SubstVisitor extends ExpressionTransformer<Void> {
  ...
  public Expression visitClassCall(ClassCallExpression expr, Void params) {
    ...
    ClassCallExpression result = new ClassCallExpression(expr.getDefinition(), expr.getLevels().subst(myLevelSubstitution), fieldSet, expr.getSortExpression().subst(myLevelSubstitution), expr.getUniverseKind());
