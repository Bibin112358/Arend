...
public class ArrayTest extends TypeCheckingTestCase {
  ...
  public void goalTest() {
    ...
    assertThatErrorsAre(Matchers.goal(1), Matchers.goal(new ClassCallExpression(Prelude.DEP_ARRAY, LevelPair.SET0, impls, UniverseKind.NO_UNIVERSES)));
  }
  ...
  public void goalTest2() {
    ...
    impls.put(Prelude.ARRAY_LENGTH, length);
    impls.put(Prelude.ARRAY_ELEMENTS_TYPE, new LamExpression(new TypedSingleDependentLink(true, null, DataCallExpression.make(Prelude.FIN, Levels.EMPTY, new SingletonList<>(length))), Nat()));
    assertThatErrorsAre(Matchers.goal(0), Matchers.goal(0), Matchers.goal(new ClassCallExpression(Prelude.DEP_ARRAY, LevelPair.SET0, impls, UniverseKind.NO_UNIVERSES)));
  }
