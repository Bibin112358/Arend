...
public class Simplifier {
      ...
      while (keepSimplifying) {
        ...
        for (var rule : simplificationRules) {
          ...
          if (path == null) {
            ...
            path = factory.appBuilder(factory.ref(meta.concat))
  ...
  private List<SimplificationRule> getSimplificationRulesForType(CoreExpression type) {
    ...
    var possibleClasses = new HashSet<>(Arrays.asList(meta.Monoid, meta.AddMonoid, meta.Semiring, meta.Ring, meta.AddGroup, meta.Group, meta.CGroup, meta.AbGroup));
    ...
    }, meta.carrier, type, typechecker, refExpr, null);
  ...
  public ConcreteExpression simplifyTypeOfExpression(ConcreteExpression expression, CoreExpression type, boolean isForward) {
    ...
    return RewriteEquationMeta.chainOfTransports(factory.ref(meta.transport, refExpr.getPLevels()),
